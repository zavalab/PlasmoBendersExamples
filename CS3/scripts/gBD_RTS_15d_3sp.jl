...
output_dir = (@__DIR__)*"/RTS_sims/15d_3sp" # Where to put the output directory
...
BendersAlg = BendersAlgorithm(graph, subgraphs[1]; tol = 3e-3, max_iters = 75, M = 0, strengthened = true, solver = solver)
...
CSV.write((@__DIR__)*"/RTS_PV_15_3b.csv", df)
