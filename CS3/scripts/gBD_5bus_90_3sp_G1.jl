...
output_dir = (@__DIR__)*"/5bus_sims/90d_3sp_G1" # Where to put the output directory
...
BendersAlg = BendersAlgorithm(graph, subgraphs[1]; tol = 5e-3, max_iters = 50, M = 0, strengthened = true, solver = solver)
...
CSV.write((@__DIR__)*"/5bus_Wind_90_3_G1.csv", df)
