...
####### BUILD DUMMY GRAPH WITH SLACKS ############
graph_slacks = OptiGraph()
...
add_subgraph!(graph_slacks, graph_DAUC)
...
for i in 1:8
    ...
    offset = (i - 1) * 12 + (day_num - 1) * 96
    build_bus_over_time(graph_STUC, 16, D_ST[:, t_range], xi_ST, 0.25, gen_ST, gen_data, gen_ST, offset = offset, add_slacks = true)
    add_subgraph!(graph_slacks, graph_STUC)
...
for i in 1:96
    ...
    add_subgraph!(graph_slacks, graph_HAED)
end

all_vars_slacks = all_variables(graph_slacks)

subgraph_set_slacks = getsubgraphs(graph_slacks)
...
for i in 1:num_days
    DAUC_dict = Dict()
    DAUC_vars = all_variables(subgraph_set_slacks[1])
    ...
    for j in 1:24
        DA_subgraph = getsubgraphs(subgraph_set_slacks[1])[j]
...
for i in 1:num_days
    for j in 1:8
        ...
        STUC_vars = all_variables(subgraph_set_slacks[1 + j])
        for k in 1:length(STUC_vars)
            STUC_dict[STUC_vars[k]] = STUC_vals[k]
        end

        for k in 1:12
            ST_subgraph = getsubgraphs(subgraph_set_slacks[1 + j])[k]
...
for i in 1:num_days
    ...
    for j in 1:96
        ...
        HAED_vars = all_variables(subgraph_set_slacks[j + 9])
        for k in 1:length(HAED_vals)
            HAED_dict[HAED_vars[k]] = HAED_vals[k]
        end

        HA_subgraph = getsubgraphs(subgraph_set_slacks[9 + j])[1]
...
for i in 1:num_days
    ...
    DAUC_vals = readdlm((@__DIR__)*"/results/32d_monolith/mipgap_results1_day$(i).csv", ',')
    for k in 1:length(DAUC_vars)
        DAUC_dict[DAUC_vars[k]] = DAUC_vals[k]
    end
    for j in 1:24
...
for i in 1:num_days
    for j in 1:8
        STUC_dict = Dict()
        STUC_vals = readdlm((@__DIR__)*"/results/32d_monolith/mipgap_results$(j + 1)_day$(i).csv", ',')
...
for i in 1:num_days
    ...
    for j in 1:96
        HAED_dict = Dict()
        HAED_vals = readdlm((@__DIR__)*"/results/32d_monolith/mipgap_results$(j + 9)_day$(i).csv", ',')
