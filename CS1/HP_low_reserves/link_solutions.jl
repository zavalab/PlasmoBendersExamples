...
"""
function link_DA_sol_to_ST(graph::OptiGraph, ST_subgraph, tau_0 = 1:4, dt = .25, gen_com_set = gen_ST; add_slacks = false)
    ...
    for (i, index) in enumerate(tau_0)
        if index <= length(getsubgraphs(DA_subgraph))
            ...
            for k in 1:num_DA_gen
                ...
                for l in 1:4
                    # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
                    if add_slacks
                        @linkconstraint(ST_subgraph, (ST_subgraphs_i[l][:bus][bus_name][:Gen_p][gen_name] + ST_subgraphs_i[l][:bus][bus_name][:Gen_m][gen_name])
                                                - (value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) + value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) <= max_ramp_up / dt + ST_subgraphs_i[l][:bus][bus_name][:link_slack_up][gen_name]
                        )
                        @linkconstraint(ST_subgraph, - (ST_subgraphs_i[l][:bus][bus_name][:Gen_p][gen_name] + ST_subgraphs_i[l][:bus][bus_name][:Gen_m][gen_name])
                                                + (value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) + value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) <= max_ramp_up / dt + ST_subgraphs_i[l][:bus][bus_name][:link_slack_up][gen_name]
                        )
                        ...
                        @linkconstraint(ST_subgraph, (ST_subgraphs_i[l][:bus][bus_name][:Gen_p][gen_name] + ST_subgraphs_i[l][:bus][bus_name][:Gen_m][gen_name])
                                                - (value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) + value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) <= max_ramp_up / dt
                        )
                        @linkconstraint(ST_subgraph, - (ST_subgraphs_i[l][:bus][bus_name][:Gen_p][gen_name] + ST_subgraphs_i[l][:bus][bus_name][:Gen_m][gen_name])
                                                + (value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) + value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) <= max_ramp_up / dt
                        )
                ...
                for l in 2:4
                    ...
                    if add_slacks
                        @linkconstraint(ST_subgraph, (bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name]) -
                                               ...
                                               max_ramp_up + bus_next[:ru_slack_up][gen_name] - bus_next[:ru_slack_down][gen_name]
                        )
                        @linkconstraint(ST_subgraph, (bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name]) -
                                                     (bus_last[:Gen_p][gen_name] + bus_last[:Gen_m][gen_name]) >=
                                                      - max_ramp_down + bus_next[:rd_slack_up][gen_name] - bus_next[:rd_slack_down][gen_name]
                        )
                    else
                        @linkconstraint(ST_subgraph, (bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name]) -
                                               (bus_last[:Gen_p][gen_name] + bus_last[:Gen_m][gen_name]) <=
                ...
                if i != 1
                    ...
                    if add_slacks
                        @linkconstraint(ST_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                            bus_last[:Gen_p][gen_name] - bus_last[:Gen_m][gen_name] <=
                            (max_startup - max_ramp_up - gen_min_cap) * value(DA_subgraph, bus_next_DA[:s][gen_name]) +
                            (max_ramp_up + gen_min_cap) * value(DA_subgraph, bus_next_DA[:x][gen_name]) -
                            gen_min_cap * value(DA_subgraph, bus_last_DA[:x][gen_name])  + bus_next[:ru_slack_up][gen_name]
                            - bus_next[:ru_slack_down][gen_name]
                        )
                        ...
                        @linkconstraint(ST_subgraph, bus_last[:Gen_p][gen_name] + bus_last[:Gen_m][gen_name] -
                            ...
                            (max_shutdown - max_ramp_down - gen_min_cap) * value(DA_subgraph, bus_next_DA[:z][gen_name]) +
                            (max_ramp_down + gen_min_cap) * value(DA_subgraph, bus_last_DA[:x][gen_name]) -
                            gen_min_cap * value(DA_subgraph, bus_next_DA[:x][gen_name]) + bus_next[:rd_slack_up][gen_name]
                            - bus_next[:rd_slack_down][gen_name]
                        )
                    else
                        @linkconstraint(ST_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                            bus_last[:Gen_p][gen_name] - bus_last[:Gen_m][gen_name] <=
                            (max_startup - max_ramp_up - gen_min_cap) * value(DA_subgraph, bus_next_DA[:s][gen_name]) +
                            (max_ramp_up + gen_min_cap) * value(DA_subgraph, bus_next_DA[:x][gen_name]) -
                            gen_min_cap * value(DA_subgraph, bus_last_DA[:x][gen_name])
                        )
                        ...
                        @linkconstraint(ST_subgraph, bus_last[:Gen_p][gen_name] + bus_last[:Gen_m][gen_name] -
                            ...
                            (max_shutdown - max_ramp_down - gen_min_cap) * value(DA_subgraph, bus_next_DA[:z][gen_name]) +
                            (max_ramp_down + gen_min_cap) * value(DA_subgraph, bus_last_DA[:x][gen_name]) -
                            gen_min_cap * value(DA_subgraph, bus_next_DA[:x][gen_name])
                        )
                    end
                end
                ...
                for (j, subgraph) in enumerate(ST_subgraphs_i)
                    # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
                    if add_slacks
                        @linkconstraint(ST_subgraph, gen_max_cap * value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:x][gen_name]) >=
                                               subgraph[:bus][bus_name][:Gen_p][gen_name] + subgraph[:bus][bus_name][:Gen_m][gen_name]
                                               - subgraph[:bus][bus_name][:max_cap_slack][gen_name]
                        )
                        @linkconstraint(ST_subgraph, gen_min_cap * value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:x][gen_name]) <=
                                               subgraph[:bus][bus_name][:Gen_p][gen_name] + subgraph[:bus][bus_name][:Gen_m][gen_name]
                                               + subgraph[:bus][bus_name][:min_cap_slack][gen_name]
                        )
                    else
                        @linkconstraint(ST_subgraph, gen_max_cap * value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:x][gen_name]) >=
                                               subgraph[:bus][bus_name][:Gen_p][gen_name] + subgraph[:bus][bus_name][:Gen_m][gen_name]
...
"""
function link_between_ST_sol(graph, ST_subgraph, ST_graphs, i; dt = .25, day_num = 1, graph_last = nothing, add_slacks = false)
    ...
    function get_s_list(ST_graphs, bus_name, gen_name, i)
        ...
        elseif i == 2
            ...
            s_list2 = Vector{Any}([value(ST_graphs[1], getsubgraphs(ST_graphs[1])[j][:bus][bus_name][:s][gen_name]) for j in 12:-1:1])
            s_list = vcat(s_list1, s_list2)
        elseif i == 3
            s_list1 = Vector{Any}([getsubgraphs(ST_graphs[3])[j][:bus][bus_name][:s][gen_name] for j in 16:-1:1])
            s_list2 = Vector{Any}([value(ST_graphs[2], getsubgraphs(ST_graphs[2])[j][:bus][bus_name][:s][gen_name]) for j in 12:-1:1])
            s_list3 = Vector{Any}([value(ST_graphs[1], getsubgraphs(ST_graphs[1])[j][:bus][bus_name][:s][gen_name]) for j in 12:-1:1])
            s_list = vcat(s_list1, s_list2, s_list3)
        else
            s_list1 = Vector{Any}([getsubgraphs(ST_graphs[i])[j][:bus][bus_name][:s][gen_name] for j in 16:-1:1])
            s_list2 = Vector{Any}([value(ST_graphs[i - 1], getsubgraphs(ST_graphs[i - 1])[j][:bus][bus_name][:s][gen_name]) for j in 12:-1:1])
            s_list3 = Vector{Any}([value(ST_graphs[i - 2], getsubgraphs(ST_graphs[i - 2])[j][:bus][bus_name][:s][gen_name]) for j in 12:-1:1])
            s_list4 = Vector{Any}([value(ST_graphs[i - 3], getsubgraphs(ST_graphs[i - 3])[j][:bus][bus_name][:s][gen_name]) for j in 12:-1:1])
    ...
    function get_z_list(ST_graphs, bus_name, gen_name, i)
        ...
        elseif i == 2
            ...
            z_list2 = Vector{Any}([value(ST_graphs[1], getsubgraphs(ST_graphs[1])[j][:bus][bus_name][:z][gen_name]) for j in 12:-1:1])
            z_list = vcat(z_list1, z_list2)
        elseif i == 3
            z_list1 = Vector{Any}([getsubgraphs(ST_graphs[3])[j][:bus][bus_name][:z][gen_name] for j in 16:-1:1])
            z_list2 = Vector{Any}([value(ST_graphs[2], getsubgraphs(ST_graphs[2])[j][:bus][bus_name][:z][gen_name]) for j in 12:-1:1])
            z_list3 = Vector{Any}([value(ST_graphs[1], getsubgraphs(ST_graphs[1])[j][:bus][bus_name][:z][gen_name]) for j in 12:-1:1])
            z_list = vcat(z_list1, z_list2, z_list3)
        else
            z_list1 = Vector{Any}([getsubgraphs(ST_graphs[i])[j][:bus][bus_name][:z][gen_name] for j in 16:-1:1])
            z_list2 = Vector{Any}([value(ST_graphs[i - 1], getsubgraphs(ST_graphs[i - 1])[j][:bus][bus_name][:z][gen_name]) for j in 12:-1:1])
            z_list3 = Vector{Any}([value(ST_graphs[i - 2], getsubgraphs(ST_graphs[i - 2])[j][:bus][bus_name][:z][gen_name]) for j in 12:-1:1])
            z_list4 = Vector{Any}([value(ST_graphs[i - 3], getsubgraphs(ST_graphs[i - 3])[j][:bus][bus_name][:z][gen_name]) for j in 12:-1:1])
    ...
    for j in 1:num_ST_gen
        ...
        if ((day_num != 1) || (i != 1))
            ...
            if i == 1
                ...
                graph_last_ST = getsubgraphs(graph_last)[9]
                graph_last_HA = HA_subgraph
            else
                ...
                graph_last_ST = ST_graphs[ST_graphs_len - 1]
                graph_last_HA = HA_subgraph
            end

            # Add linking constraint for binary variables using last STUC layers solution
            @linkconstraint(ST_subgraph, bus_next[:x][gen_name] - value(graph_last_ST, bus_last[:x][gen_name]) == bus_next[:s][gen_name] - bus_next[:z][gen_name])

            # Add ramping constraints
            # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
            if add_slacks
                @linkconstraint(ST_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(graph_last_HA, bus_last_HA[:Gen_p][gen_name]) - value(graph_last_HA, bus_last_HA[:Gen_m][gen_name]) <=
                    (max_startup - max_ramp_up - min_cap) * bus_next[:s][gen_name] +
                    (max_ramp_up + min_cap) * bus_next[:x][gen_name] - min_cap * value(graph_last_ST, bus_last[:x][gen_name])
                    + bus_next[:ru_slack_up][gen_name] - bus_next[:ru_slack_down][gen_name]
                )

                @linkconstraint(ST_subgraph, value(graph_last_HA, bus_last_HA[:Gen_p][gen_name]) + value(graph_last_HA, bus_last_HA[:Gen_m][gen_name]) -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * bus_next[:z][gen_name] +
                    (max_ramp_down + min_cap) * value(graph_last_ST, bus_last[:x][gen_name]) - min_cap * bus_next[:x][gen_name]
                    + bus_next[:rd_slack_up][gen_name] - bus_next[:rd_slack_down][gen_name]
                )
            else
                @linkconstraint(ST_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(graph_last_HA, bus_last_HA[:Gen_p][gen_name]) - value(graph_last_HA, bus_last_HA[:Gen_m][gen_name]) <=
    ...
    if (i != 1) || (day_num != 1)
        for j in 1:num_DA_gen
            ...
            if i == 1
                ...
                graph_last_HA = HA_subgraph
                graph_last_DA = DA_subgraph_last
                graph_next_DA = DA_subgraph_next
            else
                bus_next = getsubgraphs(ST_subgraph)[1][:bus][bus_name]
                bus_last = getsubgraphs(ST_graphs[ST_graphs_len - 1])[12][:bus][bus_name]
                ...
                graph_last_HA = HA_subgraph
                graph_last_DA = DA_subgraph
                graph_next_DA = DA_subgraph
            end
            ...
            if add_slacks
                @linkconstraint(ST_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(graph_last_HA, bus_last_HA[:Gen_p][gen_name]) - value(graph_last_HA, bus_last_HA[:Gen_m][gen_name]) <=
                    (max_startup - max_ramp_up - min_cap) * value(graph_next_DA, bus_next_DA[:s][gen_name]) +
                    (max_ramp_up + min_cap) * value(graph_next_DA, bus_next_DA[:x][gen_name]) -
                    min_cap * value(graph_last_DA, bus_last_DA[:x][gen_name])
                    + bus_next[:ru_slack_up][gen_name] - bus_next[:ru_slack_down][gen_name]
                )

                @linkconstraint(ST_subgraph, value(graph_last_HA, bus_last_HA[:Gen_p][gen_name]) +
                    value(graph_last_HA, bus_last_HA[:Gen_m][gen_name]) -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * value(graph_next_DA, bus_next_DA[:z][gen_name]) +
                    (max_ramp_down + min_cap) * value(graph_last_DA, bus_last_DA[:x][gen_name]) -
                    min_cap * value(graph_next_DA, bus_next_DA[:x][gen_name])
                    + bus_next[:rd_slack_up][gen_name] - bus_next[:rd_slack_down][gen_name]
                )
            else
                @linkconstraint(ST_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
...
"""
function link_sol_over_HA(graph, tau = 1; dt = .25, add_slacks = false)
    ...
    for (i, index) in enumerate(tau_range)
        ...
        for k in 1:DA_len
            ...
            if !(isequal(0, max_ramp_up))
                # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
                if add_slacks
                    @linkconstraint(HA_subgraph, (value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                  value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) -
                                                 (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] +
                                                  HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up / dt  + HA_subgraph_i[:bus][bus_name][:link_slack_up][gen_name]
                    )
                    @linkconstraint(HA_subgraph, -(value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                  value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) +
                                                  (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] +
                                                  HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up / dt
                                                  + HA_subgraph_i[:bus][bus_name][:link_slack_up][gen_name]
                    )
                else
                    @linkconstraint(HA_subgraph, (value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                  value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) -
            ...
            # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
            if add_slacks
                @linkconstraint(HA_subgraph, gen_max_cap * value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:x][gen_name]) >=
                                       HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]
                                       - HA_subgraph_i[:bus][bus_name][:max_cap_slack][gen_name]
                )
                @linkconstraint(HA_subgraph, gen_min_cap * value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:x][gen_name]) <=
                                       HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]
                                       + HA_subgraph_i[:bus][bus_name][:min_cap_slack][gen_name]
                )
            else
                @linkconstraint(HA_subgraph, gen_max_cap * value(DA_subgraph, DA_subgraph_i[:bus][bus_name][:x][gen_name]) >=
                                       HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]
            ...
            if (tau != 1)
                ...
                if i != 1
                    ...
                    bus_last_gen_p = value(HA_subgraph_last, bus_last[:Gen_p][gen_name])
                    bus_last_gen_m = value(HA_subgraph_last, bus_last[:Gen_m][gen_name])
                end
                ...
                if (tau + i - 1) % 4 == 1
                    ...
                    if add_slacks
                        @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                            ...
                            (max_startup - max_ramp_up - gen_min_cap) * value(DA_subgraph, bus_next_DA[:s][gen_name]) +
                            (max_ramp_up + gen_min_cap) * value(DA_subgraph, bus_next_DA[:x][gen_name]) -
                            gen_min_cap * value(DA_subgraph, bus_last_DA[:x][gen_name]) + bus_next[:ru_slack_up][gen_name] - bus_next[:ru_slack_down][gen_name]
                        )
                        ...
                        @linkconstraint(HA_subgraph, bus_last_gen_p + bus_last_gen_m -
                            ...
                            (max_shutdown - max_ramp_down - gen_min_cap) * value(DA_subgraph, bus_next_DA[:z][gen_name]) +
                            (max_ramp_down + gen_min_cap) * value(DA_subgraph, bus_last_DA[:x][gen_name]) -
                            gen_min_cap * value(DA_subgraph, bus_next_DA[:x][gen_name]) + bus_next[:rd_slack_up][gen_name] - bus_next[:rd_slack_down][gen_name]
                        )
                    else
                        @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                            bus_last_gen_p - bus_last_gen_m <=
                    ...
                    if add_slacks
                        @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                            bus_last_gen_p - bus_last_gen_m <= max_ramp_up + bus_next[:ru_slack_up][gen_name] -
                            bus_next[:ru_slack_down][gen_name]
                        )
                        @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                            bus_last_gen_p - bus_last_gen_m >= - max_ramp_down + bus_next[:rd_slack_up][gen_name] -
                            bus_next[:rd_slack_down][gen_name]
                        )
                    else
                        @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
        ...
        for k in 1:ST_len
            ...
            if !(isequal(0, max_ramp_up))
                # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
                if add_slacks
                    @linkconstraint(HA_subgraph, (value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                 value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) -
                                                 (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] +
                                                 HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up + HA_subgraph_i[:bus][bus_name][:link_slack_up][gen_name]# / dt
                    )
                    @linkconstraint(HA_subgraph, -(value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                 value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) +
                                                 (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] +
                    ...
                    @linkconstraint(HA_subgraph, (value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                 value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) -
                                                 (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] +
                                                 HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up
                    )
                    @linkconstraint(HA_subgraph, -(value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_p][gen_name]) +
                                                 value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:Gen_m][gen_name])) +
                                                 (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] +
            ...
            # If add_slacks, then add an additional (heavily penalized) slack variable to the constraint
            if add_slacks
                @linkconstraint(HA_subgraph, gen_max_cap * value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:x][gen_name]) >=
                                HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]
                                - HA_subgraph_i[:bus][bus_name][:max_cap_slack][gen_name]
                )
                @linkconstraint(HA_subgraph, gen_min_cap * value(ST_subgraph, ST_subgraph_i[:bus][bus_name][:x][gen_name]) <=
                                HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]
                                + HA_subgraph_i[:bus][bus_name][:min_cap_slack][gen_name]
                )
            else
            ...
            if (tau != 1)
                ...
                if add_slacks
                    @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                        bus_last_gen_p - bus_last_gen_m <=
                        (max_startup - max_ramp_up - gen_min_cap) * value(graph_next_ST, bus_next_ST[:s][gen_name]) +
                        (max_ramp_up + gen_min_cap) * value(graph_next_ST, bus_next_ST[:x][gen_name]) -
                        gen_min_cap * value(graph_last_ST, bus_last_ST[:x][gen_name]) + bus_next[:ru_slack_up][gen_name] - bus_next[:ru_slack_down][gen_name]
                    )

                    @linkconstraint(HA_subgraph, bus_last_gen_p + bus_last_gen_m -
                        bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                        (max_shutdown - max_ramp_down - gen_min_cap) * value(graph_next_ST, bus_next_ST[:z][gen_name]) +
                        (max_ramp_down + gen_min_cap) * value(graph_last_ST, bus_last_ST[:x][gen_name]) -
                        gen_min_cap * value(graph_next_ST, bus_next_ST[:x][gen_name]) + bus_next[:rd_slack_up][gen_name] - bus_next[:rd_slack_down][gen_name]
                    )
                else
                    @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                        bus_last_gen_p - bus_last_gen_m <=
                        (max_startup - max_ramp_up - gen_min_cap) * value(graph_next_ST, bus_next_ST[:s][gen_name]) +
                        (max_ramp_up + gen_min_cap) * value(graph_next_ST, bus_next_ST[:x][gen_name]) -
                        gen_min_cap * value(graph_last_ST, bus_last_ST[:x][gen_name])
                    )
                    ...
                    @linkconstraint(HA_subgraph, bus_last_gen_p + bus_last_gen_m -
                        ...
                        (max_shutdown - max_ramp_down - gen_min_cap) * value(graph_next_ST, bus_next_ST[:z][gen_name]) +
                        (max_ramp_down + gen_min_cap) * value(graph_last_ST, bus_last_ST[:x][gen_name]) -
                        gen_min_cap * value(graph_next_ST, bus_next_ST[:x][gen_name])
                    )
                end
            ...
            elseif i != 1
                ...
                if add_slacks
                    @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                        bus_last_gen_p - bus_last_gen_m <=
                        (max_startup - max_ramp_up - gen_min_cap) * value(ST_subgraph, bus_next_ST[:s][gen_name]) +
                        (max_ramp_up + gen_min_cap) * value(ST_subgraph, bus_next_ST[:x][gen_name]) -
                        gen_min_cap * value(ST_subgraph, bus_last_ST[:x][gen_name]) + bus_next[:ru_slack_up][gen_name] - bus_next[:ru_slack_down][gen_name]
                    )

                    @linkconstraint(HA_subgraph, bus_last_gen_p + bus_last_gen_m -
                        bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                        (max_shutdown - max_ramp_down - gen_min_cap) * value(ST_subgraph, bus_next_ST[:z][gen_name]) +
                        (max_ramp_down + gen_min_cap) * value(ST_subgraph, bus_last_ST[:x][gen_name]) -
                        gen_min_cap * value(ST_subgraph, bus_next_ST[:x][gen_name]) + bus_next[:rd_slack_up][gen_name] - bus_next[:rd_slack_down][gen_name]
                    )
                else
                    @linkconstraint(HA_subgraph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                        bus_last_gen_p - bus_last_gen_m <=
                        (max_startup - max_ramp_up - gen_min_cap) * value(ST_subgraph, bus_next_ST[:s][gen_name]) +
                        (max_ramp_up + gen_min_cap) * value(ST_subgraph, bus_next_ST[:x][gen_name]) -
                        gen_min_cap * value(ST_subgraph, bus_last_ST[:x][gen_name])
                    )
                    ...
                    @linkconstraint(HA_subgraph, bus_last_gen_p + bus_last_gen_m -
                        ...
                        (max_shutdown - max_ramp_down - gen_min_cap) * value(ST_subgraph, bus_next_ST[:z][gen_name]) +
                        (max_ramp_down + gen_min_cap) * value(ST_subgraph, bus_last_ST[:x][gen_name]) -
                        gen_min_cap * value(ST_subgraph, bus_next_ST[:x][gen_name])
                    )
                end
            end
        end
...
"""
function link_between_DA(DA_graph, DA_graph_set, day_num; dt = 1, HA_graph = nothing, monolithic = false)
    ...
    for the up-time constraint; s variables are sorted in opposite direction of time point (e.g., 4:0.25:0.25)
    """
    function get_s_list(DA_graph_set, bus_name, gen_name, day_num; monolithic = monolithic)
        if monolithic
            if day_num == 1
            ...
            elseif day_num == 2
                ...
                s_list2 = [value(DA_graph_set[1].parent_graph, getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:s][gen_name]) for j in 24:-1:1]
                s_list = vcat(s_list1, s_list2)
            else
                s_list1 = [getsubgraphs(DA_graph_set[3])[j][:bus][bus_name][:s][gen_name] for j in 25:-1:1]
                s_list2 = [value(DA_graph_set[2].parent_graph, getsubgraphs(DA_graph_set[2])[j][:bus][bus_name][:s][gen_name]) for j in 24:-1:1]
                s_list3 = [value(DA_graph_set[1].parent_graph, getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:s][gen_name]) for j in 24:-1:1]
                s_list = vcat(s_list1, s_list2, s_list3)
            end
        else
            if day_num == 1
            ...
            elseif day_num == 2
                s_list1 = [getsubgraphs(DA_graph_set[2])[j][:bus][bus_name][:s][gen_name] for j in 25:-1:1]
                s_list2 = [value(DA_graph_set[1], getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:s][gen_name]) for j in 24:-1:1]
                s_list = vcat(s_list1, s_list2)
            else
                s_list1 = [getsubgraphs(DA_graph_set[3])[j][:bus][bus_name][:s][gen_name] for j in 25:-1:1]
                s_list2 = [value(DA_graph_set[2], getsubgraphs(DA_graph_set[2])[j][:bus][bus_name][:s][gen_name]) for j in 24:-1:1]
                s_list3 = [value(DA_graph_set[1], getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:s][gen_name]) for j in 24:-1:1]
                s_list = vcat(s_list1, s_list2, s_list3)
            end
        end
        return s_list
    ...
    for the down-time constraint z variables are sorted in opposite direction of time point (e.g., 4:0.25:0.25)
    """
    function get_z_list(DA_graph_set, bus_name, gen_name, day_num; monolithic = monolithic)
        if monolithic
            if day_num == 1
            ...
            elseif day_num == 2
                ...
                z_list2 = [value(DA_graph_set[1].parent_graph, getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:z][gen_name]) for j in 24:-1:1]
                z_list = vcat(z_list1, z_list2)
            else
                z_list1 = [getsubgraphs(DA_graph_set[3])[j][:bus][bus_name][:z][gen_name] for j in 25:-1:1]
                z_list2 = [value(DA_graph_set[2].parent_graph, getsubgraphs(DA_graph_set[2])[j][:bus][bus_name][:z][gen_name]) for j in 24:-1:1]
                z_list3 = [value(DA_graph_set[1].parent_graph, getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:z][gen_name]) for j in 24:-1:1]
                z_list = vcat(z_list1, z_list2, z_list3)
            end
        else
            if day_num == 1
            ...
            elseif day_num == 2
                z_list1 = [getsubgraphs(DA_graph_set[2])[j][:bus][bus_name][:z][gen_name] for j in 25:-1:1]
                z_list2 = [value(DA_graph_set[1], getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:z][gen_name]) for j in 24:-1:1]
                z_list = vcat(z_list1, z_list2)
            else
                z_list1 = [getsubgraphs(DA_graph_set[3])[j][:bus][bus_name][:z][gen_name] for j in 25:-1:1]
                z_list2 = [value(DA_graph_set[2], getsubgraphs(DA_graph_set[2])[j][:bus][bus_name][:z][gen_name]) for j in 24:-1:1]
                z_list3 = [value(DA_graph_set[1], getsubgraphs(DA_graph_set[1])[j][:bus][bus_name][:z][gen_name]) for j in 24:-1:1]
                z_list = vcat(z_list1, z_list2, z_list3)
            end
        end
        return z_list
    ...
    for j in 1:num_DA_gen
        ...
        if day_num != 1
            ...
            if monolithic
                ...
                @linkconstraint(DA_graph, bus_next[:x][gen_name] - value(DA_graph_set[last_DA_graph_index].parent_graph, bus_last[:x][gen_name]) ==
                                          bus_next[:s][gen_name] - bus_next[:z][gen_name]
                )

                # Add ramping constraints
                @linkconstraint(DA_graph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(HA_graph.parent_graph, bus_last_HA[:Gen_p][gen_name]) - value(HA_graph.parent_graph, bus_last_HA[:Gen_m][gen_name]) <=
                    (max_startup - max_ramp_up - min_cap) * bus_next[:s][gen_name] +
                    (max_ramp_up + min_cap) * bus_next[:x][gen_name] - min_cap * value(DA_graph_set[last_DA_graph_index].parent_graph, bus_last[:x][gen_name])
                )

                @linkconstraint(DA_graph, value(HA_graph.parent_graph, bus_last_HA[:Gen_p][gen_name]) + value(HA_graph.parent_graph, bus_last_HA[:Gen_m][gen_name]) -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * bus_next[:z][gen_name] +
                    (max_ramp_down + min_cap) * value(DA_graph_set[last_DA_graph_index].parent_graph, bus_last[:x][gen_name]) - min_cap * bus_next[:x][gen_name]
                )
            else
                # Add constraints on binary variables
                @linkconstraint(DA_graph, bus_next[:x][gen_name] - value(DA_graph_set[last_DA_graph_index], bus_last[:x][gen_name]) ==
                                          bus_next[:s][gen_name] - bus_next[:z][gen_name]
                )

                # Add ramping constraints
                @linkconstraint(DA_graph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(HA_graph, bus_last_HA[:Gen_p][gen_name]) - value(HA_graph, bus_last_HA[:Gen_m][gen_name]) <=
                    (max_startup - max_ramp_up - min_cap) * bus_next[:s][gen_name] +
                    (max_ramp_up + min_cap) * bus_next[:x][gen_name] - min_cap * value(DA_graph_set[last_DA_graph_index], bus_last[:x][gen_name])
                )

                @linkconstraint(DA_graph, value(HA_graph, bus_last_HA[:Gen_p][gen_name]) + value(HA_graph, bus_last_HA[:Gen_m][gen_name]) -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * bus_next[:z][gen_name] +
                    (max_ramp_down + min_cap) * value(DA_graph_set[last_DA_graph_index], bus_last[:x][gen_name]) - min_cap * bus_next[:x][gen_name]
                )
            end
        end
    end
...
"""
function link_sol_over_HA_new_day(graph, graph_last; dt = .25, monolithic = true, add_slacks = false)
    ...
    for k in 1:DA_len
        ...
        bus_last = getsubgraphs(HA_subgraph_last)[1][:bus][bus_name]
        if monolithic
            bus_last_gen_p = value(HA_subgraph_last.parent_graph, bus_last[:Gen_p][gen_name])
            bus_last_gen_m = value(HA_subgraph_last.parent_graph, bus_last[:Gen_m][gen_name])
        else
            bus_last_gen_p = value(HA_subgraph_last, bus_last[:Gen_p][gen_name])
            bus_last_gen_m = value(HA_subgraph_last, bus_last[:Gen_m][gen_name])
        end
        ...
        if monolithic
            ...
            DA_x_last = value(DA_subgraph_last.parent_graph, bus_last_DA[:x][gen_name])
        else
            DA_x_next = value(DA_subgraph_next, bus_next_DA[:x][gen_name])
            DA_s_next = value(DA_subgraph_next, bus_next_DA[:s][gen_name])
            DA_z_next = value(DA_subgraph_next, bus_next_DA[:z][gen_name])
            DA_x_last = value(DA_subgraph_last, bus_last_DA[:x][gen_name])
        end
    ...
    for k in 1:ST_len
        ...
        if monolithic
            ...
            ST_x_last = value(ST_subgraph_last.parent_graph, bus_last_ST[:x][gen_name])
        else
            ST_x_next = value(ST_subgraph_next, bus_next_ST[:x][gen_name])
            ST_s_next = value(ST_subgraph_next, bus_next_ST[:s][gen_name])
            ST_z_next = value(ST_subgraph_next, bus_next_ST[:z][gen_name])
            ST_x_last = value(ST_subgraph_last, bus_last_ST[:x][gen_name])
        end
