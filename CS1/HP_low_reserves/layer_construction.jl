 ...
 * add_slacks - adds slack variables that relax all link constraints between subproblems; used for receding horizon problem
"""
function build_bus_nodes(graph, tau, D, xi, dt, gen_com_set, gen, gen_conv_set; HA::Bool = HA, add_slacks = false)
    ...
    for (i, node) in enumerate(bus)
        ...
        # if add_slacks, add slack variables for linking constraints
        if add_slacks
            @variable(node, ru_slack_up[gen_bus])
            @variable(node, ru_slack_down[gen_bus])
            @variable(node, rd_slack_up[gen_bus])
            @variable(node, rd_slack_down[gen_bus])

            @variable(node, link_slack_up[gen_bus])

            @variable(node, min_cap_slack[gen_bus])
            @variable(node, max_cap_slack[gen_bus])
        ...
        for j in 1:length(gen_com)
            ...
            if add_slacks
                @constraint(node, gen_com_max_cap[j] * x[gen_com[j]] >= Gen_p[gen_com[j]] + Gen_m[gen_com[j]] - max_cap_slack[gen_com[j]])
                @constraint(node, gen_com_min_cap[j] * x[gen_com[j]] <= Gen_p[gen_com[j]] + Gen_m[gen_com[j]] + min_cap_slack[gen_com[j]])
            else
        ...
        @variable(node, 0 <= D_shed <= 200000)

        # If add_slacks, then add an additional (heavily penalized) slack variable
        if add_slacks
            @variable(node, D_slack)
        ...
        # If add_slacks, then make sure the slack is heavily penalized in the objective
        if add_slacks && (length(gen_bus) > 0)
            obj = objective_function(node)

            add_to_expression!(obj, 20 * phi_u * (D_slack) * dt + phi_u * 10 * dt * sum(ru_slack_up[k] +
                ru_slack_down[k] + rd_slack_up[k] + rd_slack_down[k] + link_slack_up[k] +
                min_cap_slack[k] + max_cap_slack[k] for k in gen_bus)
            )
            set_objective_function(node, obj)
        end
    end
    ...
    for j in 1:length(bus)
        ...
        if add_slacks
            @linkconstraint(graph,
                ...
                bus[bus_name][:D_shed] - bus[bus_name][:D_slack]
                == D[j]
            )
        else
            @linkconstraint(graph,
 ...
 * add_slacks - adds slack variables that relax all link constraints between subproblems; used for receding horizon problem
"""
function build_bus_over_time(graph, t_len, D, xi, dt, gen_com_set, gen, gen_conv_set; offset = 0, HA::Bool = false, add_slacks = false)
    # Iterate over t_len
    for i in 1:t_len
        # Define OptiGraph for the time point
        t_graph = OptiGraph()
        # Add the necessary data to the OptiGraph
        build_bus_nodes(t_graph, i + offset, D[:, i], xi, dt, gen_com_set, gen, gen_conv_set, HA = HA, add_slacks = add_slacks)
 ...
 * add_slacks - adds slack variables that relax all link constraints between subproblems; used for receding horizon problem
"""
function link_ST_over_time(graph, dt = .25; add_slacks = false)
            ...
            for k in 1:length(gen_conv)
                ...
                if add_slacks
                    @linkconstraint(graph, bus_next[:Gen_p][gen_conv[k]] + bus_next[:Gen_m][gen_conv[k]] -
                        ...
                        (max_startup_conv[k] - max_ramp_up_conv[k] - min_cap_conv[k]) * bus_next[:s][gen_conv[k]] +
                        (max_ramp_up_conv[k] + min_cap_conv[k]) * bus_next[:x][gen_conv[k]] - min_cap_conv[k] * bus_last[:x][gen_conv[k]]
                        + bus_next[:ru_slack_up][gen_conv[k]] - bus_next[:ru_slack_down][gen_conv[k]]
                    )
                    ...
                    @linkconstraint(graph, bus_last[:Gen_p][gen_conv[k]] + bus_last[:Gen_m][gen_conv[k]] -
                        ...
                        (max_shutdown_conv[k] - max_ramp_down_conv[k] - min_cap_conv[k]) * bus_next[:z][gen_conv[k]] +
                        (max_ramp_down_conv[k] + min_cap_conv[k]) * bus_last[:x][gen_conv[k]] - min_cap_conv[k] * bus_next[:x][gen_conv[k]]
                        + bus_next[:rd_slack_up][gen_conv[k]] - bus_next[:rd_slack_down][gen_conv[k]]
                    )
                else
                    @linkconstraint(graph, bus_next[:Gen_p][gen_conv[k]] + bus_next[:Gen_m][gen_conv[k]] -
                        bus_last[:Gen_p][gen_conv[k]] - bus_last[:Gen_m][gen_conv[k]] <=
    ...
    for i in 1:8
        ...
        for j in 1:num_ST_gen
            ...
            if day_num != 1
                ...
                for k in start_index:52
                    s_list[k] = value(source_graph(owner_model(s_list[k])).parent_graph.parent_graph, s_list[k])
                    z_list[k] = value(source_graph(owner_model(z_list[k])).parent_graph.parent_graph, z_list[k])
                end
        ...
        for j in 1:num_ST_gen
            ...
            elseif day_num != 1
                ...
                # link binary variables
                @linkconstraint(graph, bus_next[:x][gen_name] - value(graph_last_ST.parent_graph, bus_last[:x][gen_name]) == bus_next[:s][gen_name] - bus_next[:z][gen_name])

                # add ramping constraints
                @linkconstraint(graph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(HA_subgraph.parent_graph, bus_last_HA[:Gen_p][gen_name]) - value(HA_subgraph.parent_graph, bus_last_HA[:Gen_m][gen_name]) <=
                    (max_startup - max_ramp_up - min_cap) * bus_next[:s][gen_name] +
                    (max_ramp_up + min_cap) * bus_next[:x][gen_name] - min_cap * value(graph_last_ST.parent_graph, bus_last[:x][gen_name])
                )

                @linkconstraint(graph, value(HA_subgraph.parent_graph, bus_last_HA[:Gen_p][gen_name]) + value(HA_subgraph.parent_graph, bus_last_HA[:Gen_m][gen_name]) -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * bus_next[:z][gen_name] +
                    (max_ramp_down + min_cap) * value(graph_last_ST.parent_graph, bus_last[:x][gen_name]) - min_cap * bus_next[:x][gen_name]
                )
            end
        end
    end
    ...
    for i in DA_iter_index
        for j in 1:num_DA_gen
            ...
            if i == 1
                ...
                @linkconstraint(graph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    value(HA_subgraph.parent_graph, bus_last_HA[:Gen_p][gen_name]) - value(HA_subgraph.parent_graph, bus_last_HA[:Gen_m][gen_name]) <=
                    (max_startup - max_ramp_up - min_cap) * bus_next_DA[:s][gen_name] +
                    (max_ramp_up + min_cap) * bus_next_DA[:x][gen_name] - min_cap * value(DA_subgraph_last.parent_graph, bus_last_DA[:x][gen_name])
                )

                @linkconstraint(graph, value(HA_subgraph.parent_graph, bus_last_HA[:Gen_p][gen_name]) + value(HA_subgraph.parent_graph, bus_last_HA[:Gen_m][gen_name]) -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * bus_next_DA[:z][gen_name] +
                    (max_ramp_down + min_cap) * value(DA_subgraph_last.parent_graph, bus_last_DA[:x][gen_name]) - min_cap * bus_next_DA[:x][gen_name]
                )
            else
                ...
                @linkconstraint(graph, bus_next[:Gen_p][gen_name] + bus_next[:Gen_m][gen_name] -
                    bus_last_HA[:Gen_p][gen_name] - bus_last_HA[:Gen_m][gen_name] <=
                    (max_startup - max_ramp_up - min_cap) * bus_next_DA[:s][gen_name] +
                    (max_ramp_up + min_cap) * bus_next_DA[:x][gen_name] - min_cap * bus_last_DA[:x][gen_name]
                )

                @linkconstraint(graph, bus_last_HA[:Gen_p][gen_name] + bus_last_HA[:Gen_m][gen_name] -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
                    (max_shutdown - max_ramp_down - min_cap) * bus_next_DA[:z][gen_name] +
                    (max_ramp_down + min_cap) * bus_last_DA[:x][gen_name] - min_cap * bus_next_DA[:x][gen_name]
                )
            end
        end
    end
        ...
        for k in 1:DA_len
            ...
            if !(isequal(0, max_ramp_up))
                @linkconstraint(graph, (DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) -
                                       (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up
                )
                @linkconstraint(graph, -(DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) +
                                       (HA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + HA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up
                )
...
function relax_slacks(graph, gen)
    buses = bus_data[:, "Bus Name"]
    ...
    for g in subgraphs
        for (i, name) in enumerate(buses)
            ...
            unfix(g[:bus][name][:D_slack])
            set_lower_bound(g[:bus][name][:D_slack], 0)

            for gen_name in gen_bus
                unfix(g[:bus][name][:ru_slack_up][gen_name])
                unfix(g[:bus][name][:ru_slack_down][gen_name])
                unfix(g[:bus][name][:rd_slack_up][gen_name])
                unfix(g[:bus][name][:rd_slack_down][gen_name])
                unfix(g[:bus][name][:min_cap_slack][gen_name])
                unfix(g[:bus][name][:max_cap_slack][gen_name])
                unfix(g[:bus][name][:link_slack_up][gen_name])

                set_lower_bound(g[:bus][name][:ru_slack_up][gen_name], 0)
                set_lower_bound(g[:bus][name][:ru_slack_down][gen_name], 0)
                set_lower_bound(g[:bus][name][:rd_slack_up][gen_name], 0)
                set_lower_bound(g[:bus][name][:rd_slack_down][gen_name], 0)
                set_lower_bound(g[:bus][name][:min_cap_slack][gen_name], 0)
                set_lower_bound(g[:bus][name][:max_cap_slack][gen_name], 0)
                set_lower_bound(g[:bus][name][:link_slack_up][gen_name], 0)
            end
        end
    end
end
...
function print_infeasible(type, day, i, graph)
    filename = (@__DIR__)*"/results/infeasible_serial.txt"
    ...
    open(filename, "a") do io
        println(io, "INFEASIBILITY in "*type*" at day $day and subproblem $i")
    end

    buses = bus_data[:, "Bus Name"]

    sgs = getsubgraphs(graph)
    for (j, g) in enumerate(sgs)
        for bus in buses
            gen_bus_bool = gen_data_conv[:, "Bus of Connection"] .== bus
            gen_bus = gen_data_conv[gen_bus_bool, "Generator Name"]

            for gen in gen_bus
                val = value(graph, g[:bus][bus][:ru_slack_up][gen])
                if val > 0
                    text = "ru_slack_up = $val at day $day subproblem $j"
                end
                val = value(graph, g[:bus][bus][:ru_slack_down][gen])
                if val > 0
                    text = "ru_slack_down = $val at day $day subproblem $j"
                end
                val = value(graph, g[:bus][bus][:rd_slack_up][gen])
                if val > 0
                    text = "rd_slack_up = $val at day $day subproblem $j"
                end
                val = value(graph, g[:bus][bus][:rd_slack_down][gen])
                if val > 0
                    text = "rd_slack_down = $val at day $day subproblem $j"
                end
                val = value(graph, g[:bus][bus][:min_cap_slack][gen])
                if val > 0
                    text = "min_cap_slack = $val at day $day subproblem $j"
                end
                val = value(graph, g[:bus][bus][:max_cap_slack][gen])
                if val > 0
                    text = "max_cap_slack = $val at day $day subproblem $j"
                end
                val = value(graph, g[:bus][bus][:link_slack_up][gen])
                if val > 0
                    text = "link_slack = $val at day $day subproblem $j"
                end
            end

            D_slack_var = g[:bus][bus][:D_slack]

            if value(graph, D_slack_var) > 0
                text = "D_slack = $(value(graph, D_slack_var)) at day $day subproblem $j"
                print_to_file(text)
            end
        end
    end
