...
function run_day1_monolith(graph_set)
    ...
    # Add constraints
    link_between_DA(graph_DAUC, [graph_DAUC], 1, monolithic = true)
    ...
    set_optimizer_attribute(graph, "MIPGap", .01)
    set_optimizer_attribute(graph, "Threads", Threads.nthreads())
    set_optimizer_attribute(graph, "PreSparsify", 1)
    set_optimizer_attribute(graph, "LogFile", (@__DIR__)*"/gurobi_logs/monolith/logfile1.log")
    set_optimizer_attribute(graph, "NodefileStart", .0005)
    set_optimizer_attribute(graph, "TimeLimit", 7200)
    ...
    for i in 1:length(subgraph_set)
        values = [value(graph, var) for var in all_variables(subgraph_set[i])]
        writedlm((@__DIR__)*"/results/32d_monolith/mipgap_results$(i)_day1.csv", values, ',')
    end
...
function run_day2_monolith(graph_set)
    ...
    link_DA_over_time(graph_DAUC, gen_data_DA)
    # Add constraints
    link_between_DA(graph_DAUC, graph_DAUC_set, 2; HA_graph = HA_subgraph_for_DA, monolithic = true)
    ...
    set_optimizer_attribute(graph, "Threads", Threads.nthreads())
    set_optimizer_attribute(graph, "PreSparsify", 1)
    set_optimizer_attribute(graph, "LogFile", (@__DIR__)*"/gurobi_logs/monolith/logfile2.log")
    set_optimizer_attribute(graph, "NodefileStart", .0005)
    set_optimizer_attribute(graph, "TimeLimit", 7200)
    ...
    for i in 1:length(subgraph_set)
        values = [value(graph, var) for var in all_variables(subgraph_set[i])]
        writedlm((@__DIR__)*"/results/32d_monolith/mipgap_results$(i)_day2.csv", values, ',')
    end
    push!(graph_set, graph)
end
...
function run_dayi_monolith(graph_set, day)
    ...
    link_DA_over_time(graph_DAUC, gen_data_DA)
    # Add constraints
    link_between_DA(graph_DAUC, graph_DAUC_set, day; HA_graph = HA_subgraph_for_DA, monolithic = true)
    ...
    set_optimizer_attribute(graph, "Threads", Threads.nthreads())
    set_optimizer_attribute(graph, "PreSparsify", 1)
    set_optimizer_attribute(graph, "LogFile", (@__DIR__)*"/gurobi_logs/monolith/logfile$(day).log")
    set_optimizer_attribute(graph, "NodefileStart", .0005)
    set_optimizer_attribute(graph, "TimeLimit", 7200)
    ...
    for i in 1:length(subgraph_set)
        values = [value(graph, var) for var in all_variables(subgraph_set[i])]
        writedlm((@__DIR__)*"/results/32d_monolith/mipgap_results$(i)_day$(day).csv", values, ',')
    end
    push!(graph_set, graph)
end
...
writedlm((@__DIR__)*"/results/monolith_run_times.csv", run_times)
